...
import { PayoffEditor } from './PayoffEditor';
import { supabase } from '../lib/supabase';
import { database } from '../lib/database';
import { exportNotesToJson } from '../lib/utils/export';
import { ImportJsonModal } from './ImportJsonModal'; // Keep this import
...
export function Menu({ onSignOut }: MenuProps) {
  const { loadProjects, projects, title, copyProject, deleteProject, switchProject, printNotes, notes, loadDeletedProjects, restoreProject, permanentlyDeleteProject, undo, canUndo, lastUndoAction } = useNoteStore();
  const { user } = useAuth();
  ...
  const handleDuplicateProject = async () => {
    try {
      ...
      alert(error instanceof Error ? error.message : 'Failed to duplicate project. Please try again.');
    }
  };
  ...
  const handleExportJson = () => {
    try {
      ...
      alert('Project exported to clipboard as JSON!');
    } catch (error) {
