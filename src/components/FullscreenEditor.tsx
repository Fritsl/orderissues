...
export const FullscreenEditor = ({
  ...
  const [unsavedYoutubeUrl, setUnsavedYoutubeUrl] = useState(note.youtube_url || '');
  const [unsavedUrl, setUnsavedUrl] = useState<string>(note.url || '');
  const [unsavedDisplayText, setUnsavedDisplayText] = useState<string>(note.url_display_text || '');
  ...
  const handleSubmit = async () => {
    try {
      ...
      await setNoteYoutubeUrl(note.id, unsavedYoutubeUrl || null);
      await setNoteUrl(note.id, unsavedUrl || null, unsavedDisplayText || null);
      
      // Clear unsavedContent after successful save
  ...
  // Update unsaved values when note changes
  useEffect(() => {
    setUnsavedYoutubeUrl(note.youtube_url || '');
    setUnsavedUrl(note.url || '');
    setUnsavedDisplayText(note.url_display_text || '');
  }, [note.youtube_url, note.url, note.url_display_text]);
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-4">
          ...
          <div className="mt-4">
            ...
            <input
              type="url"
              value={unsavedYoutubeUrl}
              onChange={(e) => setUnsavedYoutubeUrl(e.target.value)}
            ...
            <div className="mt-4">
              ...
              <input
                type="url"
                value={unsavedUrl}
                onChange={(e) => setUnsavedUrl(e.target.value)}
            ...
            <div className="mt-2">
              ...
              <input
                type="text"
                value={unsavedDisplayText}
                onChange={(e) => setUnsavedDisplayText(e.target.value)}
