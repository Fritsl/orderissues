  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'i' && !e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
        e.preventDefault();
        window.location.href = 'https://fastpresenterviwer.netlify.app';
      }
    };
...
const useExpandCollapseKeyboard = () => {
  const { expandOneLevel, collapseOneLevel, canExpandMore, canCollapseMore } = useNoteStore();
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'x' && !e.ctrlKey && !e.metaKey && canExpandMore) {
        e.preventDefault();
        expandOneLevel();
      } else if (e.key.toLowerCase() === 'z' && !e.ctrlKey && !e.metaKey && canCollapseMore) {
        e.preventDefault();
        collapseOneLevel();
      }
    };
    ...
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [expandOneLevel, collapseOneLevel, canExpandMore, canCollapseMore]);
};
