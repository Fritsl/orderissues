...
BEGIN
  ...
  RAISE NOTICE 'Updated note positions:';
  RAISE NOTICE '----------------------------------------';
  RAISE NOTICE 'ID | Content | Position';
  RAISE NOTICE '----------------------------------------';
  
  -- Use EXECUTE to format the output nicely
  FOR v_note_content, v_target_note_id, v_old_position IN
    SELECT content, id, position
    FROM notes
    WHERE project_id = v_project_id
    AND parent_id IS NOT DISTINCT FROM p_new_parent_id
    ORDER BY position
